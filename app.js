...
function fresh(){const profiles=[preset4(),preset2(),preset3()];return{schema:12,players:[],active:[],team1:[],rounds:[],series:[],endedSessions:[],pausedGames:[],profiles,activeProfileId:profiles[0].id,board:null,sessionId:uid(),started:false,startDealerId:null,ui:uiDefaults()}}
...
function normalizeData(d){
 ...
 d.ui=normalizeUi(d.ui);d.schema=12;
 ...
 const pausedSeen=new Set();d.pausedGames=d.pausedGames.map(g=>{
  ...
  const team1=Array.isArray(g?.team1)?[...new Set(g.team1.map(String))].filter(id=>active.includes(id)):[];
  const startDealerId=g?.startDealerId&&active.includes(String(g.startDealerId))?String(g.startDealerId):(active[0]||null);
  const board=g?.board&&typeof g.board==='object'?clone(g.board):{points:[initial,initial],bommerl:[0,0],over:false,dealerId:startDealerId};
  board.points=Array.isArray(board.points)&&board.points.length===2?board.points.map(n=>Number.isFinite(Number(n))?Number(n):initial):[initial,initial];
  board.bommerl=Array.isArray(board.bommerl)&&board.bommerl.length===2?board.bommerl.map(n=>Math.max(0,Number(n)||0)):[0,0];board.over=false;
  board.dealerId=board.dealerId&&active.includes(String(board.dealerId))?String(board.dealerId):startDealerId;
  return{id:String(g?.id||uid()),sessionId:String(g?.sessionId||g?.id||uid()),pausedAt:String(g?.pausedAt||g?.at||nowIso()),activeProfileId:profileId,profileId,profileName:String(g?.profileName||p?.name||'Regelprofil'),active,team1,startDealerId,board,selectedGame:String(g?.selectedGame||'__normal'),selectedModifier:String(g?.selectedModifier||'base'),selectedNormal:Number.isFinite(Number(g?.selectedNormal))?Number(g.selectedNormal):1}
 }).filter(g=>{const p=d.profiles.find(x=>x.id===g.activeProfileId),sid=String(g.sessionId);if(!p||!sid||closedSessions.has(sid)||pausedSeen.has(sid))return false;pausedSeen.add(sid);return g.active.length===p.players&&g.team1.length===p.teamSize&&g.active.filter(id=>!g.team1.includes(id)).length===p.players-p.teamSize});
...
function correctionText(r,n1,n2){if(!isCorrection(r))return'';const side=Number(r.correctionSide)===2?2:1,name=side===1?n1:n2;return`${name}: ${correctionLabel(r.correctionDelta)}`}
...
function latestRoundCanResume(sessionId=data.sessionId){const r=data.rounds[0];return !!r&&!data.started&&String(r.sessionId)===String(sessionId)&&!!r.seriesWinner}
function pausedRounds(game){return data.rounds.filter(r=>String(r.sessionId)===String(game.sessionId)&&!isCorrection(r)).length}
function pausedTeams(game){const t1=(game.team1||[]).map(pname).join(' + ')||'Seite 1',t2=(game.active||[]).filter(id=>!(game.team1||[]).includes(id)).map(pname).join(' + ')||'Seite 2';return{t1,t2}}
...
function applyCorrection(side,delta){
 ...
 side=Number(side);delta=Number(delta);if(![1,2].includes(side)||!Number.isFinite(delta)||delta===0)return alert('Bitte eine gültige Korrektur eingeben.');
 const p=profile(),teams=teamSnapshot(),before=clone(data.board),next=clampScore(Number(data.board.points[side-1])+delta,p);
 if(next===Number(data.board.points[side-1]))return alert('Diese Korrektur ändert den Punktestand nicht.');
 data.board.points[side-1]=next;const after=clone(data.board),n1=teams.team1.map(pname).join(' + ')||'Seite 1',n2=teams.team2.map(pname).join(' + ')||'Seite 2';
 data.rounds.unshift({id:uid(),sessionId:data.sessionId,at:nowIso(),profileId:p.id,profileName:p.name,type:'Korrektur',modifier:correctionLabel(delta),modifierId:'manual_correction',points:Math.abs(delta),team1:[...teams.team1],team2:[...teams.team2],winner:0,beforeBoard:before,reachedBoard:after,afterBoard:after,bommerlTo:null,bommerlCount:0,doubleBommerl:false,seriesWinner:null,dealerId:currentDealer(),correction:true,correctionSide:side,correctionDelta:delta});
 save();renderAll();renderCorrectionPanel();toast(`${side===1?n1:n2} ${correctionLabel(delta)}`);
}
...
function undoRound(){
 ...
 if(r.seriesWinner){const i=data.series.findIndex(s=>s.sessionId===r.sessionId);if(i>=0)data.series.splice(i,1);data.started=true;data.board.over=false;data.ui.page='score'}
...
function renderSetup(){
 ...
 const missingPlayers=Math.max(0,p.players-data.active.length),missingTeam=Math.max(0,p.teamSize-data.team1.length);
...
function renderPausedGames(){
 ...
 box.innerHTML=games.map(g=>{const {t1,t2}=pausedTeams(g),p=data.profiles.find(x=>x.id===g.activeProfileId),d=new Date(g.pausedAt),when=Number.isNaN(+d)?'':d.toLocaleString('de-AT',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}),rounds=pausedRounds(g),dealer=g.board?.dealerId?pname(g.board.dealerId):(g.startDealerId?pname(g.startDealerId):'—'),bombs=g.board?.bommerl?.length===2?`Bommerl ${g.board.bommerl[0]}:${g.board.bommerl[1]}`:'',meta=[p?.name||g.profileName||'Regelprofil',`${rounds} ${rounds===1?'Runde':'Runden'}`,when,`Geber: ${dealer}`,bombs].filter(Boolean).join(' · ');return`<div class="pausedGame"><div><strong>${esc(t1)} vs. ${esc(t2)}</strong><span>${esc(scorePair(g.board))}</span><small>${esc(meta)}</small></div><div class="pausedActions"><button class="btn good" data-resume-game="${esc(g.id)}" type="button">Fortsetzen</button><button class="btn secondary" data-delete-paused="${esc(g.id)}" type="button">Verwerfen</button></div></div>`}).join('');
...
function renderRoundPage(){
 ...
 const n1=data.team1.map(pname).join(' + '),n2=side2().map(pname).join(' + ');$('compactNames').textContent=`${n1} vs ${n2}`;$('compactScore').textContent=scorePair(data.board);$('compactDealer').textContent=`Geber: ${currentDealer()?pname(currentDealer()):'—'}`;$('roundProfileLabel').textContent=profile().name;
 renderRoundControls();
}
function renderRoundControls(){
 ...
 const fixed=!a.normal&&allowed.length===0,value=roundValue(),mod=modifierLabel(),parts=[a.name,mod!=='Normal'?mod:null,`${fmt(value)} ${Number(value)===1?'Punkt':'Punkte'}`].filter(Boolean);$('fixedNote').classList.toggle('hidden',!fixed);$('roundValue').textContent=fmt(value);$('roundSummary').textContent=parts.join(' · ');$('win1').textContent=(data.team1.map(pname).join(' + ')||'Seite 1')+' gewinnt';$('win2').textContent=(side2().map(pname).join(' + ')||'Seite 2')+' gewinnt';$('win1').disabled=data.board.over;$('win2').disabled=data.board.over;
}
function renderCorrectionPanel(){
 if(!$('correctionPanel'))return;const active=data.started&&ready()&&!data.board.over;
 $('openCorrection').disabled=!active;
 $('correctionName1').textContent=data.team1.map(pname).join(' + ')||'Seite 1';$('correctionName2').textContent=side2().map(pname).join(' + ')||'Seite 2';
 $('correctionScore1').textContent=data.board?.points?fmt(data.board.points[0]):'—';$('correctionScore2').textContent=data.board?.points?fmt(data.board.points[1]):'—';
...
function roundCard(r,compact=false,removable=false){const n1=r.team1.map(pname).join(' + '),n2=r.team2.map(pname).join(' + '),d=new Date(r.at).toLocaleString('de-AT',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}),corr=isCorrection(r);let ev=corr?correctionText(r,n1,n2):bommerlEventText(r,n1,n2);if(r.seriesWinner)ev+=(ev?' · ':'')+`Partie gewonnen: ${r.seriesWinner===1?n1:n2}`;const meta=corr?`Leiter-Korrektur · ${d}`:`${r.dealerId?`Geber: ${esc(pname(r.dealerId))} · `:''}${esc(r.type)} · ${esc(r.modifier||'Normal')} · ${fmt(r.points)} Punkte · ${d}`;return`<div class="match ${corr?'correctionStep':''}"><div class="matchTop"><span class="${r.winner===1?'roundWin':''}">${esc(n1)}</span><span>vs.</span><span class="${r.winner===2?'roundWin':''}">${esc(n2)}</span></div><div class="scoreChange"><strong>${esc(roundFlow(r))}</strong></div><div class="meta">${meta}</div>${compact?'':`<div class="pill">${esc(r.profileName||'Regelprofil')}</div>`}${ev?`<div class="event">${esc(ev)}</div>`:''}${removable?`<div class="roundDeleteWrap"><button class="btn danger" data-round-delete="${esc(r.id)}" type="button">Runde löschen</button></div>`:''}</div>`}
...
function renderHistory(){
 ...
 box.innerHTML=entries.map(({id,rs})=>{const sorted=rs.slice().sort((a,b)=>new Date(a.at)-new Date(b.at)),first=sorted[0],last=sorted.at(-1),n1=first.team1.map(pname).join(' + '),n2=first.team2.map(pname).join(' + '),when=new Date(first.at).toLocaleString('de-AT',{day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'}),status=roundStatusForSession(id),played=sorted.filter(r=>!isCorrection(r)).length,corrections=sorted.length-played,counts=`${played} ${played===1?'Runde':'Runden'}${corrections?` · ${corrections} Korrektur${corrections===1?'':'en'}`:''}`;let result=status.label;if(status.type==='win')result=`Gewonnen: ${status.winner===1?n1:n2}`;const board=(data.pausedGames||[]).find(x=>String(x.sessionId)===String(id))?.board||data.endedSessions.find(x=>x.sessionId===id)?.board||last.afterBoard,bombs=board?.bommerl?.length===2?`${board.bommerl[0]} : ${board.bommerl[1]} Bommerl`:'',paused=(data.pausedGames||[]).find(x=>String(x.sessionId)===String(id)),resume=paused?`<button class="btn good" data-resume-game="${esc(paused.id)}" type="button">Fortsetzen</button>`:'',undo=latestRoundCanResume(id)?`<button class="btn secondary" data-session-undo="${esc(id)}" type="button">Letzte Runde zurück</button>`:'';return`<details class="seriesGroup" ${id===data.sessionId&&data.started?'open':''}><summary><div><strong>${esc(n1)} vs. ${esc(n2)}</strong><span>${esc(first.profileName||'Regelprofil')} · ${when} · ${counts}</span></div><div class="seriesResult ${status.type==='manual'?'manual':''}">${esc(result)}${bombs?`<small>${esc(bombs)}</small>`:''}</div></summary><div class="seriesRounds"><div class="historyToolRow">${resume}${undo}<button class="btn danger" data-session-delete="${esc(id)}" type="button">Partie löschen</button></div>${sorted.map(r=>roundCard(r,true)).join('')}</div></details>`}).join('');bindHistoryActions(box);
}
...
function exportBackup(){downloadText(`schnapsen-backup-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify({format:'schnapsen-tracker-backup',version:12,exportedAt:nowIso(),data},null,2),'application/json')}
