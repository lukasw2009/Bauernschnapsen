...
function removeSessionRecords(sessionId){const id=String(sessionId);data.rounds=data.rounds.filter(r=>String(r.sessionId||r.id)!==id);data.series=data.series.filter(s=>String(s.sessionId||s.id)!==id);data.endedSessions=data.endedSessions.filter(s=>String(s.sessionId||s.id)!==id)}
...
function addRound(winner){
 ...
 if(seriesWinner){data.series.unshift({id:uid(),sessionId:data.sessionId,at:rec.at,profileId:p.id,profileName:p.name,team1:[...teams.team1],team2:[...teams.team2],winner,bommerl:[...data.board.bommerl]});data.started=false;data.ui.page='history'}
 save();renderAll();
}
...
function renderHistory(){
 ...
 box.innerHTML=entries.map(({id,rs})=>{const sorted=rs.slice().sort((a,b)=>new Date(a.at)-new Date(b.at)),first=sorted[0],last=sorted.at(-1),n1=first.team1.map(pname).join(' + '),n2=first.team2.map(pname).join(' + '),when=new Date(first.at).toLocaleString('de-AT',{day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'}),status=roundStatusForSession(id);let result=status.label;if(status.type==='win')result=`Gewonnen: ${status.winner===1?n1:n2}`;const board=data.endedSessions.find(x=>x.sessionId===id)?.board||last.afterBoard,bombs=board?.bommerl?.length===2?`${board.bommerl[0]} : ${board.bommerl[1]} Bommerl`:'',undo=latestRoundCanResume(id)?`<button class="btn secondary" data-session-undo="${esc(id)}" type="button">Letzte Runde zurück</button>`:'';return`<details class="seriesGroup" ${id===data.sessionId&&data.started?'open':''}><summary><div><strong>${esc(n1)} vs. ${esc(n2)}</strong><span>${esc(first.profileName||'Regelprofil')} · ${when} · ${sorted.length} Runden</span></div><div class="seriesResult ${status.type==='manual'?'manual':''}">${esc(result)}${bombs?`<small>${esc(bombs)}</small>`:''}</div></summary><div class="seriesRounds"><div class="historyToolRow">${undo}<button class="btn danger" data-session-delete="${esc(id)}" type="button">Partie löschen</button></div>${sorted.map(r=>roundCard(r,true)).join('')}</div></details>`}).join('');bindHistoryActions(box);
}
