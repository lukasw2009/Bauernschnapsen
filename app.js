...
function fresh(){const profiles=[preset4(),preset2(),preset3()];return{schema:9,players:[],active:[],team1:[],rounds:[],series:[],endedSessions:[],profiles,activeProfileId:profiles[0].id,board:null,sessionId:uid(),started:false,startDealerId:null,ui:uiDefaults()}}
...
function normalizeData(d){
 ...
 d.profiles=d.profiles.map(migrateProfile);
 if(!d.profiles.some(p=>p.id===d.activeProfileId))d.activeProfileId=d.profiles[0].id;
 d.players=Array.isArray(d.players)?d.players.map(p=>({id:String(p.id||uid()),name:String(p.name||'Spieler'),archived:!!p.archived})):[];
 d.active=Array.isArray(d.active)?d.active.map(String).filter(id=>d.players.some(p=>p.id===id)):[];
 d.team1=Array.isArray(d.team1)?d.team1.map(String).filter(id=>d.active.includes(id)):[];
 ...
 d.ui=Object.assign(uiDefaults(),d.ui||{});d.schema=9;
 ...
 d.rounds.forEach(r=>{r.team1=Array.isArray(r.team1)?r.team1.map(String):[];r.team2=Array.isArray(r.team2)?r.team2.map(String):[];if(r.dealerId)r.dealerId=String(r.dealerId)});
 d.series.forEach(s=>{s.team1=Array.isArray(s.team1)?s.team1.map(String):[];s.team2=Array.isArray(s.team2)?s.team2.map(String):[]});
 d.endedSessions.forEach(s=>{s.team1=Array.isArray(s.team1)?s.team1.map(String):[];s.team2=Array.isArray(s.team2)?s.team2.map(String):[]});
 return d;
}
function load(){
...
let data=load();
let selectedNormal=1, selectedModifier='base', selectedGame='__normal', roundSubmitting=false;
...
function addRound(winner){
 ...
 const p=profile(),a=selectedAnnouncement(),pts=roundValue(),teams=teamSnapshot(),before=clone(data.board),dealer=currentDealer();
 ...
 const rec={id:uid(),sessionId:data.sessionId,at:nowIso(),profileId:p.id,profileName:p.name,type:a.name,modifier:modifierLabel(),points:pts,team1:[...teams.team1],team2:[...teams.team2],winner,beforeBoard:before,reachedBoard:reached,afterBoard:after,bommerlTo,seriesWinner,dealerId:dealer};
 data.rounds.unshift(rec);
 if(seriesWinner)data.series.unshift({id:uid(),sessionId:data.sessionId,at:rec.at,profileId:p.id,profileName:p.name,team1:[...teams.team1],team2:[...teams.team2],winner,bommerl:[...data.board.bommerl]});
 save();renderAll();
...
function populateStatsFilters(){const sp=$('statsProfile');sp.innerHTML='<option value="all">Alle Regelprofile</option>'+data.profiles.map(p=>`<option value="${esc(p.id)}">${esc(p.name)}</option>`).join('');sp.value=data.ui.statsProfile;$('statsPeriod').value=data.ui.statsPeriod;$('statsSort').value=data.ui.statsSort;$('statsScope').value=data.ui.statsScope}
function renderStats(){
 ...
 const profs={};rounds.forEach(r=>{const k=r.profileName||'Unbekannt';profs[k]??={rounds:0,points:0};profs[k].rounds++;profs[k].points+=Number(r.points)||0});$('profileStats').innerHTML=Object.entries(profs).sort((a,b)=>b[1].rounds-a[1].rounds).map(([k,v])=>`<div class="stat"><span>${esc(k)}<br><span class="small">${fmt(v.points)} Punkte</span></span><strong>${v.rounds} Runden</strong></div>`).join('')||'<div class="small">Noch keine Daten.</div>';
}
...
function exportBackup(){downloadText(`schnapsen-backup-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify({format:'schnapsen-tracker-backup',version:9,exportedAt:nowIso(),data},null,2),'application/json')}
function importBackupFile(file){const r=new FileReader();r.onload=()=>{try{const raw=JSON.parse(r.result),d=raw.data||raw;if(!d||!Array.isArray(d.players)||!Array.isArray(d.profiles)||!Array.isArray(d.rounds))throw Error();if(!confirm('Backup importieren und aktuelle lokale Daten ersetzen?'))return;data=normalizeData(d);data.started=false;save();ruleDraft=null;selectedGame='__normal';selectedModifier='base';renderAll();loadRuleDraft(data.activeProfileId);toast('Backup importiert')}catch{alert('Ungültiges Schnapsen-Backup.')}};r.readAsText(file)}
...
function renderNavigation(){const page=data.ui.page;$$('.page').forEach(p=>p.classList.add('hidden'));$('page-'+page)?.classList.remove('hidden');$$('[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===page));if(page==='more')renderMore();if(page==='stats')renderStats();if(page==='history')renderHistory();if(page==='score')renderScore()}
...
function bind(){
 $('quickProfile').onchange=e=>{if(data.started){e.target.value=data.activeProfileId;return}data.activeProfileId=e.target.value;data.active=[];data.team1=[];data.startDealerId=null;data.board=blankBoard();selectedGame='__normal';selectedModifier='base';selectedNormal=profile().normalPoints[0]||1;save();ruleDraft=null;renderAll()};
 ...
 document.addEventListener('keydown',e=>{if(e.key==='Escape')closeGameMenu()});document.addEventListener('dblclick',e=>e.preventDefault(),{passive:false});document.addEventListener('gesturestart',e=>e.preventDefault(),{passive:false});
}
