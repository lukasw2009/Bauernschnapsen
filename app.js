...
function fresh(){const profiles=[preset4(),preset2(),preset3()];return{schema:11,players:[],active:[],team1:[],rounds:[],series:[],endedSessions:[],pausedGames:[],profiles,activeProfileId:profiles[0].id,board:null,sessionId:uid(),started:false,startDealerId:null,ui:uiDefaults()}}
...
function normalizeData(d){
 ...
 d.ui=normalizeUi(d.ui);d.schema=11;
...
function pausedRounds(game){return data.rounds.filter(r=>String(r.sessionId)===String(game.sessionId)).length}
...
function renderHistory(){
 ...
 box.innerHTML=entries.map(({id,rs})=>{const sorted=rs.slice().sort((a,b)=>new Date(a.at)-new Date(b.at)),first=sorted[0],last=sorted.at(-1),n1=first.team1.map(pname).join(' + '),n2=first.team2.map(pname).join(' + '),when=new Date(first.at).toLocaleString('de-AT',{day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'}),status=roundStatusForSession(id);let result=status.label;if(status.type==='win')result=`Gewonnen: ${status.winner===1?n1:n2}`;const board=(data.pausedGames||[]).find(x=>String(x.sessionId)===String(id))?.board||data.endedSessions.find(x=>x.sessionId===id)?.board||last.afterBoard,bombs=board?.bommerl?.length===2?`${board.bommerl[0]} : ${board.bommerl[1]} Bommerl`:'',paused=(data.pausedGames||[]).find(x=>String(x.sessionId)===String(id)),resume=paused?`<button class="btn good" data-resume-game="${esc(paused.id)}" type="button">Fortsetzen</button>`:'',undo=latestRoundCanResume(id)?`<button class="btn secondary" data-session-undo="${esc(id)}" type="button">Letzte Runde zurück</button>`:'';return`<details class="seriesGroup" ${id===data.sessionId&&data.started?'open':''}><summary><div><strong>${esc(n1)} vs. ${esc(n2)}</strong><span>${esc(first.profileName||'Regelprofil')} · ${when} · ${sorted.length} Runden</span></div><div class="seriesResult ${status.type==='manual'?'manual':''}">${esc(result)}${bombs?`<small>${esc(bombs)}</small>`:''}</div></summary><div class="seriesRounds"><div class="historyToolRow">${resume}${undo}<button class="btn danger" data-session-delete="${esc(id)}" type="button">Partie löschen</button></div>${sorted.map(r=>roundCard(r,true)).join('')}</div></details>`}).join('');bindHistoryActions(box);
}
...
function exportBackup(){downloadText(`schnapsen-backup-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify({format:'schnapsen-tracker-backup',version:11,exportedAt:nowIso(),data},null,2),'application/json')}
...
function renderGameMenu(){$('editGameSetup').disabled=!data.started;$('undoRound').disabled=!data.started||!data.rounds[0]||data.rounds[0].sessionId!==data.sessionId;$('pauseGame').disabled=!data.started||data.board.over;$('newSeries').disabled=!data.started;$('endGame').disabled=!data.started||data.board.over}
